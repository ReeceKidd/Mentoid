...
module.exports = async function updateAreasOfInterest(userID, age, areasOfInterest) {
    ...
    */
    ...
    await axios.post(updateMentorPreferencesURL, {
        areasOfInterestMentoring: areasOfInterest,
        ...
        prefferedEducationMentoring: prefferedEducation,
        minimumAgeMentoring: minimumAge,
        maximumAgeMentoring: maximumAge,
        userID: userID
    }).catch(error => {
...
function getMinimumAge(age) {
    return Math.floor(Math.random() * age) + 16
}

function getMaximumAge(minimumAge) {
    return Math.floor(Math.random() * 120) + minimumAge
}