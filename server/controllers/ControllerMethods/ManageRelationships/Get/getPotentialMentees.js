...
module.exports = getPotentialMentors = (req, res) => {
    ...
    logger.warn(req.params.userID + ' is attempting to get their potential mentees with ' + JSON.stringify(req.params))
    ...
    User.findById(req.params.userID, function (err, user) {
        if (err) {
            ...
            res.status(500)
            res.send({
                message: 'Could not get potential mentors because of server error',
                error: 'Server error'
            })
        } 
        if(!user){
        ...
        logger.warn(user.userName + ' successfully retrieved: potential mentors: ' +
        ' -Number of potential mentors: ' + user.potentialMentees.length) 
        res.status(200).send({
