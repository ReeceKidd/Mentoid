            ...
            User.findById(req.body.mentorID,
                function (err, mentor) {
                    if (err) {
                        ...
                        logger.error(mentor.userName + ' maximum number of mentees: ' + mentor.mentorSettings.maxNumberOfMentees)
                        logger.error(mentor.userName + ' current number of mentees: ' + mentor.mentees.length)
                        logger.error(mentor.userName + ' current mentees: ' + JSON.stringify(mentor.mentees, null, 2))
                        if (mentor.mentees.length <= mentor.mentorSettings.maxNumberOfMentees) {
                            logger.error(mentor.userName + ' number of current mentees is less than their maximum')
                        } else {
                            logger.error(mentor.userName + ' number of current mentees is greater than their maximum')
                        }
                        ...
                        logger.error(mentor.userName + ' number of mentees on waiting list: ' + mentor.potentialMentees.length)
                        let isAlreadyOnList = false
                        mentor.potentialMentees.forEach(potentialMentee => {
                            if (potentialMentee._id == req.body.userID) {
                                logger.error(user.userName + ' is already on ' + mentor.userName + ' waiting list.')
                        ...
                        logger.error(user.userName + ' is not on ' + mentor.userName + ' adding to wait list.')
